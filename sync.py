#!/usr/bin/env python3
"""
Obsidian Vault Sync - Convert Claude sessions to Obsidian notes
...
FORMAT   = 2   # bump to force re-conversion of every transcript
...
def convert(path, manifest):
    """Convert a single .jsonl transcript to markdown note."""
    sid   = Path(path).stem
    ...
    outdir = CONV_DIR / ym; outdir.mkdir(parents=True, exist_ok=True)
    ...
    fm = ['---',
          ...
          f'date: {date}', f'project: {proj["label"]}']
    if hub: fm.append(f'hub: "[[{hub}]]"')
    fm += [f'session: {sid}', f'turns: {len(turns)}',
           f'tags: [claude/conversation, project/{proj["tag"]}]', '---', '']
    ...
    for kind, md in turns:
        if kind == 'user':
            ...
            if last != 'asst': L += ['', '## \U0001F916 Claude', '']
            L += [md, '']; last = 'asst'
        else:
            L += [md, '']
...
def main():
    ...
    save_manifest(manifest); build_index(manifest)
    total = len([m for m in CONV_DIR.rglob('*.md') if not m.name.startswith('_')])
    print(f"\nVault: {VAULT}")
    print(f"Scanned {len(files)} transcripts | {new} new/updated | {total} conversation notes")
